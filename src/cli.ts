...
export function validateTranslations() {
  ...
  const baseLocale = baseIndex !== -1 && args[baseIndex + 1] ? args[baseIndex + 1] : 'en';

  const i18nDir = join(process.cwd(), 'i18n');
  const files = readdirSync(i18nDir).filter(f => f.endsWith('.json'));

  const baseContent = JSON.parse(readFileSync(join(i18nDir, `${baseLocale}.json`), 'utf-8'));
  ...
  files.forEach(file => {
    ...
    const currentContent = JSON.parse(readFileSync(join(i18nDir, file), 'utf-8'));
    ...
    if (missingKeys.length > 0 || extraKeys.length > 0) {
      ...
      extraKeys.forEach(k => console.warn(`  ⚠️  Extra key (not in base): ${k}`));
    }
  });
